...
import {CANVAS_HEIGHT, CANVAS_WIDTH, gameManager} from "./sketch.js";
...
class GameScore {
  ...
  scoreCheck(ball, goal1, goal2) {
    // Player 1 scores
    if (
      ball.body.position.x - ball.r >=
        CANVAS_WIDTH - goal2.areaWidth + goal2.botBarW / 2 &&
      ball.body.position.y >= CANVAS_HEIGHT - goal2.areaHeight
    ) {
      if (!this.sloMo) {
    ...
    // Player 2 scores
    if (
      ball.body.position.x + ball.r <= goal1.areaWidth - goal1.botBarW &&
      ball.body.position.y >= CANVAS_HEIGHT - goal1.areaHeight
    ) {
      if (!this.sloMo) {
        this.previousFrameCount = frameCount;
        frameRate(20);
